...
/**
 * This class was modified from Meka to speed up training
 *
...
public class BR extends ProblemTransformationMethod implements MultiLabelDrawable {
    ...
    public Instances  transformInstances(Instances data,Instances shell,int labelToKeep,int[] labelIndices) throws Exception {
        Instances shellCopy = new Instances(shell);
        boolean order10 = false;
        if (data.attribute(labelIndices[labelToKeep]).value(0).equals("1")) {
            order10 = true;
        }
        for (int j = 0; j < shellCopy.numInstances(); j++) {
            if (order10) {
                shellCopy.instance(j).setValue(0, 1 - data.instance(j).value(labelIndices[labelToKeep]));
            } else {
                shellCopy.instance(j).setValue(0, data.instance(j).value(labelIndices[labelToKeep]));
            }
        }
        return shellCopy;
    }
    ...
    public void buildClassifier(Instances D) throws Exception {
        ...
        //get label indices
        int L = D.classIndex();
        int[] labelIndices = new int[L];
        for(int i = 0;i<L;++i){
            labelIndices[i] = i;
        }
        //remove all but one label attribute
        int[] removeInds =new int[labelIndices.length-1];
        for(int i = 0;i<removeInds.length;++i){
            removeInds[i]=labelIndices[i];
        }
        Instances shell = F.remove(D,removeInds,false);        
        ...
        for(int j = 0; j < L; j++) {
            ...
            Instances D_j = transformInstances(D,shell,j,labelIndices);
            D_j.setClassIndex(0);
